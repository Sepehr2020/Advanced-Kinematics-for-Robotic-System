...
%4.1 Inverse Kinematics
  %Define the link dimensions (DH parameters)
    ...
    alpha=P_ee(4)*pi/180; beta=P_ee(5)*pi/180; gamma=P_ee(6)*pi/180;    % uncomment for FK
    ...
    eq(1) = [100*cos(theta1) + 100*cos(theta1)*cos(theta3) - 200*cos(theta1)*sin(theta3) - d2*sin(theta1) == P_ee(1) - lee*noap(1,1)];
    eq(2) = [100*sin(theta1) + 100*cos(theta3)*sin(theta1) - 200*sin(theta1)*sin(theta3) + d2*cos(theta1) == P_ee(2) - lee*noap(2,1)];
    eq(3) = [100 - 100*sin(theta3) - 200*cos(theta3) == P_ee(3) - lee*noap(3,1)];
    ans = solve(eq, [theta1,d2,theta3]);
    theta1 = ans.theta1;
    d2 = ans.d2;
    theta3 = ans.theta3;
    ...
    joints = [theta1;  d2; theta3; theta4; theta5; theta6];
...
for i=1:length(D(1,:))
     ...
     newV{3} = R_03*V{3};                                          %Find new orientation of 3rd revolute joint
     newV{3} = newV{3} + repmat(P_03,[1 length(newV{3}(1,:))]);    %Find new position of 3rd revolute
     newV{4} = R_06*V{4};                                          %Find new orientation of components
     newV{4} = newV{4} + repmat(P_06,[1 length(newV{4}(1,:))]);    %Find new position of components
     newV{5} = R_06*V{5};                                          %Find new orientation of components
     newV{5} = newV{5} + repmat(P_06,[1 length(newV{5}(1,:))]);    %Find new position of components
     newV{6} = R_06*V{6};                                          %Find new orientation of components
     newV{6} = newV{6} + repmat(P_06,[1 length(newV{6}(1,:))]);    %Find new position of components
     newV{7} = R_06*V{7};                                          %Find new orientation of components
     newV{7} = newV{7} + repmat(P_06,[1 length(newV{7}(1,:))]);    %Find new position of components
     newV{8} = R_06*V{8};                                          %Find new orientation of end-effector
     newV{8} = newV{8} + repmat(P_06,[1 length(newV{8}(1,:))]);    %Find new position of end-effector
     ...
     newV{13} = R_02*V{13};                                         %Find new orientation of prismatic
     newV{13} = newV{13} + repmat(P_02,[1 length(newV{13}(1,:))]);  %Find new position of prismatic
